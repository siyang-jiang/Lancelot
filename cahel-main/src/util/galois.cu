...
namespace cahel::util
{
    ...
    {
        if ((coeff_count_power < get_power_of_two(CAHEL_POLY_MOD_DEGREE_MIN)) ||
            coeff_count_power > get_power_of_two(CAHEL_POLY_MOD_DEGREE_MAX))
        {
        ...
        permutation_tables_ = std::shared_ptr<std::shared_ptr<uint32_t>>(std::vector<std::shared_ptr<uint32_t>>(coeff_count_).data());
    }
