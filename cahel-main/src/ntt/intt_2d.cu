...
inplace_inwt_radix8_phase2(uint64_t *inout,
    ...
    for (size_t i = blockIdx.x * blockDim.x + threadIdx.x; i < (n / 8 * coeff_mod_size);
        ...
        const DModulus *modulus_table = modulus;
        uint64_t modulus = modulus_table[twr_idx].value();
        // DMulUIntMod inv_degree_mod = inv_degree_modulo[twr_idx];
...
#pragma unroll
        for (size_t j = 0; j < 8; j++) {
            *(data_ptr + n_init + t / 4 * j) = samples[j];
            // multiply_and_reduce_shoup(samples[j], inv_degree_mod, modulus);
        }
    }
}
...
#pragma unroll
        for (size_t j = 0; j < 8; j++) {
            *(data_ptr + n_init + t / 4 * j) = multiply_and_reduce_shoup(samples[j], scale[twr_idx], modulus);
            // multiply_and_reduce_shoup(samples[j], inv_degree_mod, modulus);
        }
    }
}
...
inplace_inwt_radix8_phase1_include_temp_mod(uint64_t *inout,
    ...
    for (size_t i = blockIdx.x * blockDim.x + threadIdx.x; i < (n / 8 * coeff_mod_size);
        ...
        // base address
        uint64_t *data_ptr = inout + twr_idx * n;
        const DMulUIntMod *psi = itwiddles + n * twr_idx2;
        const DModulus *modulus_table = modulus;
...
#pragma unroll
        for (size_t j = 0; j < 8; j++) {
            *(data_ptr + n_init + t / 4 * j) = multiply_and_reduce_shoup(samples[j], scale[twr_idx2], modulus);
            // multiply_and_reduce_shoup(samples[j], inv_degree_mod, modulus);
        }
    }
}
