...
namespace cahel
{
    void Modulus::set_value(uint64_t value)
    {
        ...
        else if ((value >> CAHEL_MOD_BIT_COUNT_MAX != 0) || (value == 1))
        {
    ...
    {
        if (poly_modulus_degree > CAHEL_POLY_MOD_DEGREE_MAX || poly_modulus_degree < CAHEL_POLY_MOD_DEGREE_MIN ||
            get_power_of_two(static_cast<uint64_t>(poly_modulus_degree)) < 0)
        {
            throw invalid_argument("poly_modulus_degree is invalid");
        }
        if (bit_sizes.size() > CAHEL_COEFF_MOD_COUNT_MAX)
        {
            throw invalid_argument("bit_sizes is invalid");
        }
        if (accumulate(
                bit_sizes.cbegin(), bit_sizes.cend(), CAHEL_USER_MOD_BIT_COUNT_MIN,
                [](int a, int b)
                { return max(a, b); }) > CAHEL_USER_MOD_BIT_COUNT_MAX ||
            accumulate(bit_sizes.cbegin(), bit_sizes.cend(), CAHEL_USER_MOD_BIT_COUNT_MAX, [](int a, int b)
                       { return min(a, b); }) < CAHEL_USER_MOD_BIT_COUNT_MIN)
        {
            throw invalid_argument("bit_sizes is invalid");
        }
