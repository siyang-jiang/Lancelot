...
__global__ void multiply_scalar_rns_poly(const uint64_t *operand,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        size_t twr = tid / poly_degree;
        uint64_t mod_value = modulus[twr].value();
        DMulUIntMod scalar_value = scalar[twr];

        result[tid] = multiply_and_reduce_shoup(operand[tid], scalar_value, mod_value);
    }
}
...
__global__ void multiply_temp_mod_and_add_rns_poly(const uint64_t *operand1,
                                                   ...
                                                   const size_t dnum,
                                                   const size_t alpha,
                                                   const DMulUIntMod *bigP_mod_q) {
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
         tid < n * dnum * alpha;
         tid += blockDim.x * gridDim.x) {
        size_t twr = tid / (alpha * n);
        size_t mod_idx = tid / n;
        uint64_t qi = modulus[mod_idx].value();
        DMulUIntMod factor = bigP_mod_q[mod_idx];
        uint64_t tmp = multiply_and_reduce_shoup(operand1[tid], factor, qi);
...
__global__ void sub_and_scale_single_mod_poly(const uint64_t *operand1,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        result[tid] = multiply_and_reduce_shoup(temp, scale, modulus);
    }
}
...
__global__ void sub_and_scale_rns_poly(const uint64_t *operand1,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        add_uint64_uint64(operand1[tid], temp, temp);
        result[tid] = multiply_and_reduce_shoup(temp, scale[twr], mod_value);
    }
}

__global__ void bfv_add_timesQ_overt_kernel(uint64_t *ct,
                                            const uint64_t *pt,
                                            DMulUIntMod negQl_mod_t,
                                            const DMulUIntMod *tInv_mod_q,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        DMulUIntMod tInv_mod_qi = tInv_mod_q[twr];

        uint64_t m = pt[tid % n];
        uint64_t mQl_mod_t = multiply_and_reduce_shoup(m, negQl_mod_t, t);
        ct[tid] += multiply_and_reduce_shoup(mQl_mod_t, tInv_mod_qi, qi);
...
__global__ void bfv_sub_timesQ_overt_kernel(uint64_t *ct,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        uint64_t m = pt[tid % n];
        uint64_t mQl_mod_t = multiply_and_reduce_shoup(m, negQl_mod_t, t);
        ct[tid] = ct[tid] + qi - multiply_and_reduce_shoup(mQl_mod_t, tInv_mod_qi, qi);
        if (ct[tid] >= qi) ct[tid] -= qi;
    }
}

/** For BFV, cipher + ceil(m*Q/t);
 * @brief (llround((m * (Q mod t)) / t) + m * (qi // t)) mod qi
 */
__global__ void multiply_add_plain_with_scaling_variant_kernel(uint64_t *cipher_data,
                                                               const uint64_t *plain_data,
                                                               const uint64_t coeff_modulus_mod_plain_modulus,
                                                               const uint64_t plain_upper_half_threshold,
                                                               const uint64_t plain_modulus,
                                                               const DMulUIntMod *coeff_div_plain_modulus,
                                                               const DModulus *coeff_modulus,
                                                               const uint64_t poly_degree,
                                                               const uint64_t coeff_mod_size) {
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
         tid < poly_degree * coeff_mod_size;
        ...
        // plain * q_mod_t + [(t+1) / 2], 128-bit addition
        temp = add_uint128_uint64(temp, plain_upper_half_threshold);
        // {plain * q_mod_t + [(t+1) / 2] } / t, 128 / 64 -> 64
        divide_uint128_uint64_generic(temp, plain_modulus, quotient);

        temp = multiply_uint64_uint64(pt, qi_div_t);
        temp = add_uint128_uint64(temp, quotient.lo);
        temp = add_uint128_uint64(temp, cipher_data[tid]);
        temp.lo = barrett_reduce_uint128_uint64(temp, qi.value(), qi.const_ratio());
        cipher_data[tid] = temp.lo;
    }
}

/** For BFV, cipher - ceil(m*q/t);
 * @brief (llround((m * (Q mod t)) / t) - m * (Q / t)) mod qi
 */
__global__ void multiply_sub_plain_with_scaling_variant_kernel(uint64_t *cipher_data,
                                                               const uint64_t *plain_data,
                                                               const uint64_t coeff_modulus_mod_plain_modulus,
                                                               const uint64_t plain_upper_half_threshold,
                                                               const uint64_t plain_modulus,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        temp.lo = barrett_reduce_uint128_uint64(temp, qi.value(), qi.const_ratio());
        // cipher = (cipher - res) % q
        cipher_data[tid] = sub_uint64_uint64_mod(cipher_data[tid], temp.lo, qi.value());
    }
}
