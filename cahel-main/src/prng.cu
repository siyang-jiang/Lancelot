 ...
 */
void __device__ salsa20_gpu(uint8_t *out, const size_t outlen, const uint64_t nonce, const uint8_t *key, const size_t keylen)
{
    int k;
    uint32_t *mem;
    uint32_t x[16], j[16];
 ...
 */
__global__ void sample_ternary_poly(uint64_t *out, const uint8_t *prng_seed, const DModulus *modulus, const uint64_t poly_degree, const uint64_t coeff_mod_size)
{
    uint8_t tmp[64];
    uint64_t flag;
         ...
         tid += blockDim.x * gridDim.x)
    {
        ...
        salsa20_gpu(tmp, 64, tid % poly_degree, prng_seed, cahel::util::global_variables::prng_seed_byte_count);
        tmp[0] = tmp[0] % 3;
 ...
 */
__global__ void sample_uniform_poly(uint64_t *out, const uint8_t *prng_seed, const DModulus *modulus, const uint64_t poly_degree, const uint64_t coeff_mod_size)
{
    uint8_t tmp[64];
         ...
         tid += blockDim.x * gridDim.x)
    {
        ...
        uint64_t max_multiple = max_random - barrett_reduce_uint64_uint64(max_random, mod.value(), mod.const_ratio()[1]) - 1;

        auto start_pos = tid * 8;
        salsa20_gpu(tmp, 64, tid, prng_seed, cahel::util::global_variables::prng_seed_byte_count);
        tries++;
        while (index < 8)
        {
            while (rnd[index] > max_multiple)
            {
                salsa20_gpu(tmp, 64, tid + tries * poly_degree * coeff_mod_size, prng_seed, cahel::util::global_variables::prng_seed_byte_count);
                tries++;
            }
 ...
 * @param[in] coeff_mod_size The number of coeff modulus
 */
__global__ void sample_error_poly(uint64_t *out, const uint8_t *prng_seed, const DModulus *modulus, const uint64_t poly_degree, const uint64_t coeff_mod_size)
{
    uint8_t tmp[64];
    int32_t cbd;
    uint64_t flag;
