...
__global__ void bconv_mult_kernel(uint64_t *dst,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        dst[tid] = multiply_and_reduce_shoup(
                src[tid],
                scale[i],
                modulus);
    }
...
__global__ void bconv_mult_unroll2_kernel(uint64_t *dst,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        out_x = multiply_and_reduce_shoup(in_x, scale_factor, modulus);
        out_y = multiply_and_reduce_shoup(in_y, scale_factor, modulus);
...
__global__ void bconv_matmul_unroll4_kernel(uint64_t *dst,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
         tid < (n * obase_size + unroll_number - 1) / unroll_number;
         tid += blockDim.x * gridDim.x) {
        const size_t degree_idx = unroll_number * (tid / obase_size);
        const size_t out_prime_idx = tid % obase_size;

        uint128_t4 accum = base_convert_acc_unroll4(xi_qiHatInv_mod_qi,
                                                    s_QHatModp, out_prime_idx,
                                                    n,
                                                    ibase_size,
                                                    degree_idx);
...
void DBaseConverter::bConv_BEHZ(uint64_t *dst, const uint64_t *src, size_t n) const {
    ...
    temp.acquire(allocate<uint64_t>(Global(), ibase_size * n));
...
base_convert_matmul_hps_kernel(uint64_t *dst,
                               const uint64_t *xi_qiHatInv_mod_qi,
                               const uint64_t *qiHat_mod_pj,
                               const uint64_t *v_Q_mod_pj,
                               const double *qiInv,
                               const DModulus *ibase, uint64_t ibase_size,
                               const DModulus *obase, uint64_t obase_size,
                               uint64_t n) {
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        double_t accum_frac = base_convert_acc_frac(xi_qiHatInv_mod_qi,
                                                    qiInv,
                                                    n,
                                                    ibase_size,
                                                    degree_idx);
...
__global__ static void base_convert_matmul_hps_unroll2_kernel(uint64_t *dst,
                                                              const uint64_t *xi_qiHatInv_mod_qi,
                                                              const uint64_t *qiHat_mod_pj,
                                                              const uint64_t *v_Q_mod_pj,
                                                              const double *qiInv,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        double_t2 accum_frac = base_convert_acc_frac_unroll2(xi_qiHatInv_mod_qi,
                                                             qiInv,
                                                             n,
                                                             ibase_size,
                                                             degree_idx);
...
[[maybe_unused]] __global__ static void
base_convert_matmul_hps_unroll4_kernel(uint64_t *dst,
                                       const uint64_t *xi_qiHatInv_mod_qi,
                                       const uint64_t *qiHat_mod_pj,
                                       const uint64_t *v_Q_mod_pj,
                                       const double *qiInv,
                                       const DModulus *ibase,
                                       uint64_t ibase_size,
                                       const DModulus *obase,
                                       uint64_t obase_size,
                                       uint64_t n) {
    constexpr const int unroll_number = 4;
    extern __shared__ uint64_t s_qiHat_mod_pj[];
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        double_t4 accum_frac = base_convert_acc_frac_unroll4(xi_qiHatInv_mod_qi,
                                                             qiInv,
                                                             n,
                                                             ibase_size,
                                                             degree_idx);
...
void DBaseConverter::bConv_HPS(uint64_t *dst,
                               const uint64_t *src,
                               size_t n) const {
    size_t ibase_size = ibase_.size();
    size_t obase_size = obase_.size();
...
__global__ static void exact_convert_array_kernel(uint64_t *dst,
                                                  const uint64_t *src,
                                                  const DModulus *ibase,
                                                  const uint64_t ibase_size,
                                                  const DModulus *obase,
                                                  const uint64_t obase_size,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        for (uint64_t i = 0; i < ibase_size; i++) {
            ...
            yi = multiply_and_reduce_shoup(src[tid + i * poly_degree],
            ...
            add_uint128_uint128(multiply_uint64_uint64(yi, base_change_matrix[i]),
                                inner_prod,
                                inner_prod);

            q_mod_t = barrett_reduce_uint128_uint64({q_mod_t, ibase_prod[ibase_size - i - 1]},
                                                    t.value(), t.const_ratio());
...
void DBaseConverter::exact_convert_array(uint64_t *dst, const uint64_t *src, const uint64_t poly_degree) const {
    ...
    exact_convert_array_kernel<<<gridDimGlb, blockDimGlb>>>(dst, src,
                                                            ibase_.base(), ibase_size,
                                                            obase_.base(), obase_size,
                                                            ibase_.big_modulus(),
                                                            ibase_.QHatInvModq(),
                                                            QHatModp(),
                                                            poly_degree, reduction_threshold);
}

__global__ static void modup_copy_partQl_kernel(
        uint64_t *t_mod_up, const uint64_t *cks, size_t size_Ql_n, size_t size_QlP_n, size_t size_alpha_n) {
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x; tid < size_Ql_n; tid += blockDim.x * gridDim.x) {
        size_t beta_idx = tid / size_alpha_n;
        t_mod_up[beta_idx * size_QlP_n + tid] = cks[tid];
    }
}

__global__ static void modup_bconv_single_p_kernel(uint64_t *dst,
                                                   const uint64_t *src_raw,
                                                   const uint64_t *src_normal_form,
                                                   size_t in_prime_idx, size_t n,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        if (out_prime_idx != in_prime_idx) {
            ...
            uint64_t result;
            if (in_prime > out_prime) result = barrett_reduce_uint64_uint64(coeff, out_prime, barret_ratio);
            else result = coeff;
            dst[tid] = result;
        } else {
...
__global__ static void bconv_matmul_padded_unroll2_kernel(uint64_t *dst,
                                                          const uint64_t *xi_qiHatInv_mod_qi,
                                                          const uint64_t *qiHat_mod_pj,
                                                          const DModulus *ibase, uint64_t ibase_size,
                                                          const DModulus *obase, uint64_t obase_size,
...
[[maybe_unused]] __global__ static void
bconv_matmul_padded_unroll4_kernel(uint64_t *dst,
                                   const uint64_t *xi_qiHatInv_mod_qi,
                                   const uint64_t *qiHat_mod_pj,
                                   const DModulus *ibase, uint64_t ibase_size,
                                   const DModulus *obase, uint64_t obase_size,
...
void DRNSTool::modup(uint64_t *dst,
                     const uint64_t *cks,
    ...
    t_cks.acquire(allocate<uint64_t>(Global(), size_Ql_n));

#ifdef CAHEL_PROFILE
    CUDATimer modup_timer("modup");
    modup_timer.start();
#endif
    ...
    if (alpha == 1) {
        ...
        if (scheme == scheme_type::ckks || scheme == scheme_type::bgv) {
            ...
            nwt_2d_radix8_backward_scale(t_cks.get(), cks, ntt_tables, size_Ql, 0,
                                         partQlHatInv_mod_Ql_concat_.get());
        }

        // copy partQl to t_mod_up
        modup_copy_partQl_kernel<<<n * size_Ql / blockDimGlb.x, blockDimGlb>>>(
                dst, cks, size_Ql_n, size_QlP_n, alpha * n);
    }

    for (size_t beta_idx = 0; beta_idx < beta; beta_idx++) {
        uint64_t *t_mod_up_i = dst + beta_idx * size_QlP_n;
        ...
        if (alpha == 1) {
            ...
            if (scheme == scheme_type::ckks || scheme == scheme_type::bgv) {
                modup_bconv_single_p_kernel<<<gridDimGlb, blockDimGlb>>>(
                        t_mod_up_i,
                        cks + startPartIdx * n,
                        t_cks.get() + startPartIdx * n,
                        startPartIdx, n,
                        base_QlP_.base(), size_QlP);
            ...
            auto qiHat_mod_pj = base_part_Ql_to_compl_part_QlP_conv.QHatModp();
            ...
            if (scheme == scheme_type::bfv) {
                ...
                bconv_mult_kernel<<<gridDimGlb, blockDimGlb>>>(
                        t_cks.get() + startPartIdx * n,
                        cks + startPartIdx * n,
                        ibase.QHatInvModq(),
                        ibase.base(), ibase_size,
                        n);
            }

            const int unroll_factor = 2;
            gridDimGlb = n * obase_size / blockDimGlb.x / unroll_factor;
            bconv_matmul_padded_unroll2_kernel<<<gridDimGlb, blockDimGlb,
            sizeof(uint64_t) * obase_size * ibase_size>>>(
                    t_mod_up_i,
                    t_cks.get() + startPartIdx * n,
                    qiHat_mod_pj,
                    ibase.base(), ibase_size,
                    obase.base(), obase_size,
                    n,
        ...
        if (scheme == scheme_type::ckks || scheme == scheme_type::bgv) {
            ...
            nwt_2d_radix8_forward_inplace_include_special_mod_exclude_range(
                    t_mod_up_i, ntt_tables,
                    size_QlP, 0,
                    size_QP, size_P,
                    startPartIdx, endPartIdx);
        } else if (scheme == scheme_type::bfv) {
            nwt_2d_radix8_forward_inplace_include_special_mod(
                    t_mod_up_i, ntt_tables, size_QlP, 0, size_QP, size_P);
        } else {
            throw invalid_argument("unsupported scheme");
        }
    }
 ...
 */
__global__ static void bgv_moddown_kernel(uint64_t *dst,
                                          const uint64_t *cx,
                                          const uint64_t *delta,
                                          const uint64_t *cp_mod_t,
                                          const DMulUIntMod *P_mod_qi,
                                          const DMulUIntMod *PInv_mod_qi,
                                          const DMulUIntMod PInv_mod_t,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        uint64_t temp = multiply_and_reduce_shoup(cp_mod_t[tid % n], PInv_mod_t, t);
        uint64_t correction = multiply_and_reduce_shoup(temp, P_mod_qi[i], qi);
        temp = sub_uint64_uint64_mod(cx[tid], delta[tid], qi);
        temp = add_uint64_uint64_mod(temp, correction, qi);
        dst[tid] = multiply_and_reduce_shoup(temp, PInv_mod_qi[i], qi);
    }
}

__global__ static void
moddown_kernel(uint64_t *dst,
               const uint64_t *cx,
               const uint64_t *delta,
               const DModulus *modulus,
               const DMulUIntMod *mod_switch_factor,
               size_t n,
               size_t size_Ql) {
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
         tid < n * size_Ql;
        ...
        dst[tid] = barrett_reduce_uint128_uint64(temp, mod.value(), mod.const_ratio());
    }
}

__global__ static void moddown_bconv_single_p_kernel(uint64_t *dst,
                                                     const uint64_t *src,
                                                     size_t n,
                                                     const DModulus *base_QlP, uint64_t size_QlP) {
...
void DRNSTool::moddown(
    ...
    delta.acquire(allocate<uint64_t>(Global(), size_QlP * n));

    if (scheme == scheme_type::ckks) {
        // Transform cx_i[P] to normal domain
        nwt_2d_radix8_backward_inplace_include_special_mod(cx_i,
                                                           ntt_tables,
                                                           size_P_,
                                                           size_Ql,
                                                           size_QP_,
    ...
    if (scheme == scheme_type::bgv) {
        base_P_to_Ql_conv_.bConv_BEHZ(delta.get(),
                                      cx_i + size_Ql_n,
                                      n);

        Pointer<uint64_t> temp_t;
        temp_t.acquire(allocate<uint64_t>(Global(), n));

        base_P_to_t_conv_.bConv_BEHZ(temp_t.get(),
                                     cx_i + size_Ql_n,
                                     n);

        bgv_moddown_kernel<<<size_Ql_n / blockDimGlb.x, blockDimGlb>>>(
                ct_i, cx_i, delta.get(), temp_t.get(),
                bigP_mod_q(), bigPInv_mod_q(), bigPInv_mod_t_,
                ntt_tables.modulus(), size_Ql,
                t_.value(), n);

        nwt_2d_radix8_forward_inplace(ct_i,
                                      ntt_tables,
                                      size_Ql,
                                      0);
    } else { // BFV and CKKS
        ...
        if (scheme == scheme_type::ckks) {
            ...
            nwt_2d_radix8_forward_inplace(delta.get(),
                                          ntt_tables,
                                          size_Ql,
                                          0);
        }
        ...
        moddown_kernel<<<size_Ql_n / blockDimGlb.x, blockDimGlb>>>(
                ct_i, cx_i, delta.get(),
                ntt_tables.modulus(), bigPInv_mod_q_.get(),
                n, size_Ql);
    }
}
 ...
 */
void DRNSTool::moddown_from_NTT(uint64_t *ct_i,
                                uint64_t *cx_i,
                                const DNTTTable &ntt_tables,
                                const scheme_type &scheme) const {
    size_t n = n_;
    ...
    Pointer<uint64_t> delta;
    delta.acquire(allocate<uint64_t>(Global(), size_Ql_n));

#ifdef CAHEL_PROFILE
    CUDATimer moddown_timer("moddown");
    moddown_timer.start();
#endif
    ...
    if (alpha == 1) {
        ...
        moddown_bconv_single_p_kernel<<<gridDimGlb, blockDimGlb>>>(
                delta.get(),
                cx_i + size_Ql_n,
                n,
                base_QlP_.base(), size_QlP);
