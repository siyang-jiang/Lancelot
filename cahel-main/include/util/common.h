...
namespace cahel
{
    namespace util
    {
        ...
        struct IsUInt32
            : std::conditional<
                  std::is_integral<T>::value && cuda::std::is_unsigned<T>::value && (sizeof(T) == sizeof(std::uint32_t)),
                  std::true_type, std::false_type>::type
        {
        };
...
#pragma nv_diag_suppress 173
#else
#pragma GCC diagnostic push
#pragma diag_suppress 173, 186
#endif
                // Both integer types
...
#pragma GCC diagnostic pop
        ...
        [[nodiscard]] inline constexpr uint64_t reverse_bits(uint64_t operand) noexcept
        {
// Temporarily disable UB warnings when `if constexpr` is not available.
#ifndef CAHEL_USE_IF_CONSTEXPR
...
#pragma diag_suppress 63
            ...
            return static_cast<uint64_t>(reverse_bits(static_cast<std::uint32_t>(operand >> 32))) |
                   (static_cast<uint64_t>(reverse_bits(static_cast<std::uint32_t>(operand & uint64_t(0xFFFFFFFF)))) << 32);
#ifdef __NVCC_DIAG_PRAGMA_SUPPORT__
#pragma nv_diagnostic pop
#else
#pragma GCC diagnostic pop
...
#pragma clang diagnostic pop
        ...
        [[nodiscard]] inline int get_significant_bit_count(std::uint64_t value)
        {
            ...
            CAHEL_MSB_INDEX_UINT64(&result, value);
            return static_cast<int>(result + 1);
        }

        [[nodiscard]] inline bool is_hex_char(char hex)
        {
            if (hex >= '0' && hex <= '9')
            {
                return true;
        ...
        [[nodiscard]] inline char nibble_to_upper_hex(int nibble)
        {
            if (nibble < 10)
            {
        ...
        [[nodiscard]] inline int hex_to_nibble(char hex)
        {
            if (hex >= '0' && hex <= '9')
            {
        ...
        [[nodiscard]] inline int get_hex_string_bit_count(const char *hex_string, int char_count)
        {
