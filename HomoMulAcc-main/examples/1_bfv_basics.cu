...
{
    std::cout << std::endl
              << "Testing BFV encode & decode" << std::endl;
    for (size_t poly_modulus_degree = 8192; poly_modulus_degree <= 32768; poly_modulus_degree = poly_modulus_degree * 2)
    {
        EncryptionParameters parms(scheme_type::bfv);
...
void example_bfv_encrypt_decrypt()
{
    ...
    for (size_t poly_modulus_degree = 4096; poly_modulus_degree <= 32768; poly_modulus_degree = poly_modulus_degree * 2)
    {
        EncryptionParameters parms(scheme_type::bfv);
        std::cout << "testing poly_modulus_degree = " << poly_modulus_degree << std::endl;
        parms.set_poly_modulus_degree(poly_modulus_degree);
...
{
    std::cout << std::endl
              << "Testing BFV (sym, asym) cipher Add" << std::endl;
    for (size_t poly_modulus_degree = 4096; poly_modulus_degree <= 32768; poly_modulus_degree = poly_modulus_degree * 2)
    {
        EncryptionParameters parms(scheme_type::bfv);
        std::cout << "testing poly_modulus_degree = " << poly_modulus_degree << std::endl;
        ...
        CAHELGPUPlaintext dec_asym_plain(context);
        ...
        CAHELGPUCiphertext sym_cipher_copy(sym_cipher);
        ...
        CAHELGPUCiphertext destination2(context);
...
{
    std::cout << std::endl
              << "Testing BFV (sym, asym) cipher sub" << std::endl;
    for (size_t poly_modulus_degree = 4096; poly_modulus_degree <= 32768; poly_modulus_degree = poly_modulus_degree * 2)
    {
        EncryptionParameters parms(scheme_type::bfv);
        std::cout << "testing poly_modulus_degree = " << poly_modulus_degree << std::endl;
        ...
        secret_key.gen_publickey(context, public_key);
        public_key.encrypt_asymmetric(context, plain_matrix, asym_cipher, false);
        CAHELGPUCiphertext asym_cipher_copy(context);
...
{
    std::cout << std::endl
              << "Testing BFV (sym, asym) cipher mul" << std::endl;
    for (size_t poly_modulus_degree = 4096; poly_modulus_degree <= 32768; poly_modulus_degree = poly_modulus_degree * 2)
    {
        EncryptionParameters parms(scheme_type::bfv);
        std::cout << "testing poly_modulus_degree = " << poly_modulus_degree << std::endl;
...
{
    std::cout << std::endl
              << "Testing BFV (sym, asym) cipher square" << std::endl;
    for (size_t poly_modulus_degree = 4096; poly_modulus_degree <= 32768; poly_modulus_degree = poly_modulus_degree * 2)
    {
        EncryptionParameters parms(scheme_type::bfv);
        std::cout << "testing poly_modulus_degree = " << poly_modulus_degree << std::endl;
        ...
        secret_key.encrypt_symmetric(context, plain_matrix, sym_cipher, false);
        // CAHELGPUCiphertext destination(context);
        square_inplace(context, sym_cipher);
        relinearize_inplace(context, sym_cipher, relin_keys);
        secret_key.decrypt(context, sym_cipher, dec_plain);
        batchEncoder.decode(context, dec_plain, dec_res);
        ...
        secret_key.gen_publickey(context, public_key);
        public_key.encrypt_asymmetric(context, plain_matrix, asym_cipher, false);
        square_inplace(context, asym_cipher);
        relinearize_inplace(context, asym_cipher, relin_keys);
        secret_key.decrypt(context, asym_cipher, dec_asym_plain);
...
{
    std::cout << std::endl
              << "Testing BFV (sym, asym) cipher Add Plain" << std::endl;
    for (size_t poly_modulus_degree = 4096; poly_modulus_degree <= 32768; poly_modulus_degree = poly_modulus_degree * 2)
    {
        EncryptionParameters parms(scheme_type::bfv);
        std::cout << "testing poly_modulus_degree = " << poly_modulus_degree << std::endl;
...
{
    std::cout << std::endl
              << "Testing BFV (sym, asym) cipher sub Plain" << std::endl;
    for (size_t poly_modulus_degree = 4096; poly_modulus_degree <= 32768; poly_modulus_degree = poly_modulus_degree * 2)
    {
        EncryptionParameters parms(scheme_type::bfv);
        std::cout << "testing poly_modulus_degree = " << poly_modulus_degree << std::endl;
...
{
    std::cout << std::endl
              << "Testing BFV (sym, asym) cipher mul many Plain" << std::endl;
    for (size_t poly_modulus_degree = 4096; poly_modulus_degree <= 32768; poly_modulus_degree = poly_modulus_degree * 2)
    {
        EncryptionParameters parms(scheme_type::bfv);
        std::cout << "testing poly_modulus_degree = " << poly_modulus_degree << std::endl;
...
{
    std::cout << std::endl
              << "Testing BFV (sym, asym) cipher mul one Plain" << std::endl;
    for (size_t poly_modulus_degree = 4096; poly_modulus_degree <= 32768; poly_modulus_degree = poly_modulus_degree * 2)
    {
        EncryptionParameters parms(scheme_type::bfv);
        std::cout << "testing poly_modulus_degree = " << poly_modulus_degree << std::endl;
...
void example_bfv_exponentiate()
{
    uint64_t exponent[4] = {2, 4, 8, 15};
    size_t index = 0;

    std::cout << std::endl
              << "Testing BFV (sym, asym) cipher exponentiate" << std::endl;
    for (size_t poly_modulus_degree = 4096; poly_modulus_degree <= 32768; poly_modulus_degree = poly_modulus_degree * 2)
    {
        EncryptionParameters parms(scheme_type::bfv);
        std::cout << "testing poly_modulus_degree = " << poly_modulus_degree << std::endl;
        ...
        secret_key.encrypt_symmetric(context, plain_matrix, sym_cipher, false);
        // CAHELGPUCiphertext destination(context);
        exponentiate_inplace(context, sym_cipher, exponent[index], relin_keys);
        secret_key.decrypt(context, sym_cipher, dec_plain);
        batchEncoder.decode(context, dec_plain, dec_res);

        bool correctness = true;
        for (size_t idx = 0; idx < slot_count; idx++)
            correctness &= dec_res[idx] == pow(idx, exponent[index], plainModulus.value());
        if (!correctness)
            throw std::logic_error("Error in exponentiate symmetric");

        secret_key.gen_publickey(context, public_key);
        public_key.encrypt_asymmetric(context, plain_matrix, asym_cipher, false);
        // CAHELGPUCiphertext destination2(context);
        exponentiate_inplace(context, asym_cipher, exponent[index], relin_keys);
        secret_key.decrypt(context, asym_cipher, dec_asym_plain);
        ...
        for (size_t idx = 0; idx < slot_count; idx++)
            correctness &= dec_res[idx] == pow(idx, exponent[index], plainModulus.value());
        if (!correctness)
            throw std::logic_error("Error in exponentiate asymmetric");
...
{
    std::cout << std::endl
              << "Testing BFV (sym, asym) cipher rotate column" << std::endl;
    for (size_t poly_modulus_degree = 4096; poly_modulus_degree <= 32768; poly_modulus_degree = poly_modulus_degree * 2)
    {
        EncryptionParameters parms(scheme_type::bfv);
        std::cout << "testing poly_modulus_degree = " << poly_modulus_degree << std::endl;
...
{
    std::cout << std::endl
              << "Testing BFV (sym, asym) cipher rotate row" << std::endl;
    for (size_t poly_modulus_degree = 4096; poly_modulus_degree <= 32768; poly_modulus_degree = poly_modulus_degree * 2)
    {
        EncryptionParameters parms(scheme_type::bfv);
        std::cout << "testing poly_modulus_degree = " << poly_modulus_degree << std::endl;
