...
namespace cahel {
    ...
    enum class scheme_type : std::uint8_t {
        none = 0, // // No scheme set; cannot be used for encryption
        bgv = 1, // Brakerski-Gentry-Vaikuntanathan scheme
        bfv = 2, // Brakerski/Fan-Vercauteren scheme
        ckks = 3 // Cheon-Kim-Kim-Song scheme
    };

    // Techniques for multiplication
    enum class mul_tech_type : std::uint8_t {
        none = 0, // No technique set, if the scheme is not BFV
        behz = 1, // BEHZ
        hps = 2, // HPS
        hps_overq = 3, // HPS over Q
    ...
    public:
        ...
        */
        inline void set_coeff_modulus(const std::vector<Modulus> &coeff_modulus) {
            ...
            } else if (coeff_modulus.size() > CAHEL_COEFF_MOD_COUNT_MAX ||
                       coeff_modulus.size() < CAHEL_COEFF_MOD_COUNT_MIN) {
        ...
        */
        ...
        /**
        Enables access to private members of cahel::EncryptionParameters for C
        */
    ...
    private:
        ...
        inline void compute_parms_id() {
            ...
            *param_data_ptr++ = static_cast<uint64_t>(special_modulus_size_);
            ...
            cahel::util::HashFunction::hash(param_data.data(), total_uint64_count, parms_id_);
        ...
        void load_members(std::istream &stream) {
            ...
            try {
                ...
                if (poly_modulus_degree64 > CAHEL_POLY_MOD_DEGREE_MAX) {
                ...
                // Only check for upper bound; lower bound is zero for scheme_type::none
                if (coeff_modulus_size64 > CAHEL_COEFF_MOD_COUNT_MAX) {
