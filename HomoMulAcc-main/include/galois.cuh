...
typedef struct CAHELGPUGaloisTool {
    int coeff_count_power_ = 0;
    std::size_t coeff_count_ = 0;
    std::vector<size_t> galois_indexs_{};
    std::vector<uint32_t> galois_elts_{};
    std::vector<cahel::util::Pointer<uint32_t>> permutation_tables_;
    std::vector<cahel::util::Pointer<uint64_t>> index_raw_tables_; // only used by bfv
    bool is_bfv_;

    CAHELGPUGaloisTool(const std::vector<uint32_t> &galois_elts, int coeff_count_power, bool is_bfv = false) {
        if ((coeff_count_power < cahel::util::get_power_of_two(CAHEL_POLY_MOD_DEGREE_MIN)) ||
            coeff_count_power > cahel::util::get_power_of_two(CAHEL_POLY_MOD_DEGREE_MAX)) {
        ...
        for (std::size_t idx{0}; idx < galois_elts_size; idx++) {
            permutation_tables_[idx].acquire(cahel::util::allocate<uint32_t>(cahel::util::Global(), coeff_count_));
            auto galois_elt = galois_elts_.at(idx);
            auto temp_ptr = u32temp.data();
            for (size_t i = coeff_count_; i < coeff_count_ << 1; i++) {
                uint32_t reversed = cahel::util::reverse_bits(static_cast<uint32_t>(i), coeff_count_power_ + 1);
                ...
                *temp_ptr++ = cahel::util::reverse_bits(static_cast<uint32_t>(index_raw), coeff_count_power_);
            }
        ...
        if (is_bfv_) {
            ...
            for (std::size_t idx = 0; idx < galois_elts_size; idx++) {
                index_raw_tables_[idx].acquire(cahel::util::allocate<uint64_t>(cahel::util::Global(), coeff_count_));
                auto galois_elt = galois_elts_.at(idx);
    ...
    CAHELGPUGaloisTool &operator=(CAHELGPUGaloisTool &&move) noexcept {
    ...
    */
    [[nodiscard]] static inline std::size_t get_index_from_elt(uint32_t galois_elt) {
#ifdef CAHEL_DEBUG
        if (!(galois_elt & 1))
        {
...
#endif
        return static_cast<std::size_t>((galois_elt - 1) >> 1);
    }
    ...
    get_indexs_from_elts(const std::vector<uint32_t> &galois_elts) {
        ...
        for (uint32_t galois_elt: galois_elts) {
#ifdef CAHEL_DEBUG
            if (!(galois_elt & 1))
            {
                throw std::invalid_argument("galois_elt is not valid");
