...
typedef struct CAHELGPUPlaintext {
    ...
    CAHELGPUPlaintext() = default;

    explicit CAHELGPUPlaintext(const CAHELGPUContext &context) {
        auto cahel_context = context.cpu_context_;
        auto &context_data = (cahel::CAHELContext::ContextData &) (cahel_context->get_context_data(0));
        ...
        else if (parms.scheme() == cahel::scheme_type::ckks)
            coeff_modulus_size_ = context.coeff_mod_size_;
        else
            throw std::invalid_argument("invalid FHE scheme.");

        if (coeff_modulus_size_ == 0) {
            throw std::invalid_argument("CAHELGPUContext not inited yet.");
        }
        poly_modulus_degree_ = context.poly_degree_;
        // Malloc memory
        data_.acquire(
                cahel::util::allocate<uint64_t>(cahel::util::Global(), coeff_modulus_size_ * poly_modulus_degree_));
    }

    // copy constructor
    CAHELGPUPlaintext(const CAHELGPUPlaintext &copy) {
    ...
    // move constructor
    CAHELGPUPlaintext(CAHELGPUPlaintext &&source) noexcept {
    ...
    // copy assignment
    CAHELGPUPlaintext &operator=(const CAHELGPUPlaintext &copy) {
        if (this != (CAHELGPUPlaintext *) &copy) {
            parms_id_ = copy.parms_id_;
            chain_index_ = copy.chain_index_;
            poly_modulus_degree_ = copy.poly_modulus_degree_;
            coeff_modulus_size_ = copy.coeff_modulus_size_;
    ...
    // move assignment
    CAHELGPUPlaintext &operator=(CAHELGPUPlaintext &&source) noexcept {
        if (this != (CAHELGPUPlaintext *) &source) {
            parms_id_ = source.parms_id_;
            chain_index_ = source.chain_index_;
            poly_modulus_degree_ = source.poly_modulus_degree_;
            coeff_modulus_size_ = source.coeff_modulus_size_;
    ...
    [[nodiscard]] std::string to_string() const {
        ...
        return cahel::util::poly_to_hex_string(data(), coeff_modulus_size_ * poly_modulus_degree_, 1);
    }
    ...
    void load(const CAHELGPUContext &context, std::istream &stream) {
        // Verify parameters
        if (!context.cpu_context_->parameters_set()) {
            throw std::invalid_argument("encryption parameters are not set correctly");
        }
        CAHELGPUPlaintext new_data(context);
        ...
        try {
            ...
            auto total_uint64_count =
                    cahel::util::mul_safe(new_data.poly_modulus_degree_, new_data.coeff_modulus_size_);

            new_data.data_.acquire(cahel::util::allocate<uint64_t>(cahel::util::Global(), total_uint64_count));
