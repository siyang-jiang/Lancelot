...
void
poly_infty_norm_coeffmod(const uint64_t *poly, size_t coeff_count, size_t coeff_uint64_count, const uint64_t *modulus,
                         uint64_t *result) {
 ...
 */
void CAHELGPUPublicKey::encrypt_zero_asymmetric(const CAHELGPUContext &context, CAHELGPUCiphertext &cipher,
                                                size_t chain_index, bool is_ntt_form) const {
    auto cahel_context = context.cpu_context_;
    auto &context_data = cahel_context->get_context_data(chain_index);
    ...
    random_bytes(context.prng_seed(), cahel::util::global_variables::prng_seed_byte_count);
    ...
    if (is_ntt_form) {
        for (size_t i = 0; i < cipher.size_; i++) {
            ...
            if (parms.scheme() == scheme_type::bgv) {
                ...
                multiply_scalar_rns_poly<<<gridDimGlb, blockDimGlb>>>(ci, context.plain_modulus_shoup(), base_rns, ci,
        ...
        error.acquire(allocate<uint64_t>(Global(), coeff_mod_size * poly_degree));
 ...
 */
void CAHELGPUPublicKey::encrypt_zero_asymmetric_internal(const CAHELGPUContext &context, CAHELGPUCiphertext &cipher,
    ...
    if (parms.scheme() == cahel::scheme_type::ckks || parms.scheme() == scheme_type::bgv) {
        is_ntt_form = true;
    ...
    auto prev_index_id = context.get_previous_index(chain_index);
    if (prev_index_id == chain_index) {
        // Does not require modulus switching
        return encrypt_zero_asymmetric(context, cipher, chain_index, is_ntt_form);
    } else {
        CAHELGPUCiphertext temp_cipher;
        encrypt_zero_asymmetric(context, temp_cipher, prev_index_id, is_ntt_form);
        auto &rns_tool = context.gpu_rns_tool_vec()[prev_index_id];
        // moddown
        ...
        for (size_t i = 0; i < temp_cipher.size(); i++) {
            ...
            context.gpu_rns_tool_vec()[1].moddown(ct_i,
                                                  cx_i,
 ...
 * @param[in] save_seed Save random seed in ciphertext
 */
void CAHELGPUPublicKey::encrypt_asymmetric(const CAHELGPUContext &context, const CAHELGPUPlaintext &plain,
                                           CAHELGPUCiphertext &cipher, bool save_seed) {
    auto cahel_context = context.cpu_context_;
    auto &context_data = (CAHELContext::ContextData &) (cahel_context->get_context_data(0)); // i.e. 0 is key_param_id
    auto &parms = (EncryptionParameters &) (context_data.parms());
    auto scheme = parms.scheme();
    if (scheme == cahel::scheme_type::bfv) {
    ...
    } else if (scheme == cahel::scheme_type::ckks) {
        if (!plain.is_ntt_form()) {
        ...
        // [c0 + plaintext, c1]
        auto &ckks_context_data = cahel_context->get_context_data(plain.chain_index());
    ...
    } else if (scheme == cahel::scheme_type::bgv) {
        if (plain.is_ntt_form()) {
        ...
        auto &bgv_context_data = cahel_context->get_context_data(context.get_first_index());
        ...
        plain_copy.acquire(allocate<uint64_t>(Global(), bgv_coeff_mod_size * poly_degree));
 ...
 * @param[in] context CAHELGPUContext
 */
void CAHELGPUSecretKey::gen_secretkey(const CAHELGPUContext &context) {
    ...
    secret_key_array_.acquire(allocate<uint64_t>(Global(), poly_degree * coeff_mod_size));
...
void CAHELGPUSecretKey::compute_secret_key_array(const CAHELGPUContext &context, size_t max_power) {
    ...
    auto cahel_context = context.cpu_context_;
    auto &key_context_data = (CAHELContext::ContextData &) (cahel_context->get_context_data(0));
    auto &parms = (EncryptionParameters &) (key_context_data.parms());
    auto &coeff_modulus = parms.coeff_modulus();
    auto poly_degree = parms.poly_modulus_degree();
    auto coeff_mod_size = coeff_modulus.size();
    auto base_rns = context.gpu_rns_tables().modulus();
    ...
    new_secret_key_array.acquire(allocate<uint64_t>(Global(), max_power * poly_degree * coeff_mod_size));
...
void CAHELGPUSecretKey::gen_publickey(const CAHELGPUContext &context, CAHELGPUPublicKey &pk, bool save_seed) const {
    encrypt_zero_symmetric(context, pk.pk_, 0, true, save_seed); // use key_param_id (i.e., 0)
    pk.pk_.chain_index() = 0;
    pk.pk_.parms_id() = (context.cpu_context_)->key_parms_id();
}

void CAHELGPUSecretKey::generate_one_kswitch_key(const CAHELGPUContext &context, uint64_t *new_key,
                                                 CAHELGPURelinKey &relin_keys, bool save_seed) const {
    // Extract encryption parameters.
    auto cpu_context = context.cpu_context_;
    auto &key_context_data = (CAHELContext::ContextData &) (cpu_context->get_context_data(0));
    auto &key_parms = (EncryptionParameters &) (key_context_data.parms());
    ...
    auto bigP_mod_q = context.gpu_rns_tool_vec()[0].bigP_mod_q();

    // Every pk_ = [P_{w,q}(s^2)+(-(as+e)), a]
    relin_keys.public_keys_.resize(dnum);
    relin_keys.public_keys_ptr_.acquire(allocate<uint64_t *>(Global(), dnum));
    ...
    multiply_temp_mod_and_add_rns_poly<<<gridDimGlb, blockDimGlb>>>(
            ...
            base_rns,
            relin_keys.public_keys_ptr_.get(),
            poly_degree, dnum, alpha, bigP_mod_q);

    // Set the gen flag
    relin_keys.gen_flag_ = true;
    relin_keys.parms_id() = cpu_context->key_parms_id();
}

void CAHELGPUSecretKey::gen_relinkey(const CAHELGPUContext &context, CAHELGPURelinKey &relin_key, bool save_seed) {
    // Check to see if secret key and public key have been generated
    if (!gen_flag_) {
        throw logic_error("cannot generate relinearization keys for unspecified secret key");
    }
...
void
CAHELGPUSecretKey::create_galois_keys(const CAHELGPUContext &context, CAHELGPUGaloisKey &galois_keys, bool save_seed) const {
    // Check to see if secret key and public key have been generated
    if (!gen_flag_) {
        throw logic_error("cannot generate relinearization keys for unspecified secret key");
    }
    ...
    rotated_secret_key.acquire(allocate<uint64_t>(Global(), key_mod_size * poly_degree));
    ...
    for (size_t galois_elt_idx{0}; galois_elt_idx < galois_keys.relin_key_num_; galois_elt_idx++) {
        ...
        CAHELGPURelinKey relin_key(context);
    ...
    galois_keys.parms_id_ = cahel_context->key_parms_id();
 ...
 */
void CAHELGPUSecretKey::encrypt_zero_symmetric(const CAHELGPUContext &context,
    ...
    if (is_ntt_form) {
        if (parms.scheme() == scheme_type::bgv) {
            // noise = te instead of e in BGV
            multiply_scalar_rns_poly<<<gridDimGlb, blockDimGlb>>>(context.in(), context.plain_modulus_shoup(), base_rns,
                                                                  context.in(), poly_degree, coeff_mod_size);
        }
 ...
 * @param[in] save_seed Save random seed in ciphertext
 */
void CAHELGPUSecretKey::encrypt_symmetric(const CAHELGPUContext &context,
                                          const CAHELGPUPlaintext &plain,
                                          CAHELGPUCiphertext &cipher,
                                          bool save_seed) const {
    auto cahel_context = context.cpu_context_;
    auto &context_data = (CAHELContext::ContextData &) (cahel_context->get_context_data(
            0)); // Use key_parm_id for obtaining scheme
    auto &parms = (EncryptionParameters &) (context_data.parms());
    auto scheme = parms.scheme();
    bool is_ntt_form = false;
    if (scheme == cahel::scheme_type::bfv) {
        ...
        // [c0 + plaintext, c1]
        auto &ckks_context_data = (CAHELContext::ContextData &) (cahel_context->get_context_data(plain.chain_index()));
        auto &ckks_parms = (EncryptionParameters &) (ckks_context_data.parms());
        auto poly_degree = ckks_parms.poly_modulus_degree();
        ...
        auto &bgv_context_data = (CAHELContext::ContextData &) (
                cahel_context->get_context_data(context.get_first_index()));
        auto &bgv_parms = (EncryptionParameters &) (bgv_context_data.parms());
        auto poly_degree = bgv_parms.poly_modulus_degree();
        auto bgv_coeff_mod_size = bgv_parms.coeff_modulus().size();
        auto base_rns = context.gpu_rns_tables().modulus();
...
void CAHELGPUSecretKey::ckks_decrypt(const CAHELGPUContext &context, const CAHELGPUCiphertext &encrypted,
...
void CAHELGPUSecretKey::bfv_decrypt(const CAHELGPUContext &context, const CAHELGPUCiphertext &encrypted,
                                    CAHELGPUPlaintext &destination) {
    ...
    inner_prod.acquire(allocate<uint64_t>(Global(), coeff_mod_size * poly_degree));
    CUDA_CHECK(cudaMemset(inner_prod.get(), 0UL, coeff_mod_size * poly_degree * sizeof(uint64_t)));
    Pointer<uint64_t> temp;
    temp.acquire(allocate<uint64_t>(Global(), coeff_mod_size * poly_degree));
    ...
    nwt_2d_radix8_backward_inplace(inner_prod.get(),
                                   context.gpu_rns_tables(),
                                   coeff_mod_size,
                                   0);

    // finally, c_0 = c_0 + c_1
    add_rns_poly<<<gridDimGlb, blockDimGlb>>>(c0,
                                              inner_prod.get(),
                                              base_rns,
                                              inner_prod.get(),
                                              poly_degree,
    ...
    if (mul_tech == mul_tech_type::behz) {
        ...
        context.gpu_rns_tool_vec()[chain_index].behz_decrypt_scale_and_round(inner_prod.get(),
                                                                             temp.get(),
                                                                             context.gpu_rns_tables(),
                                                                             coeff_mod_size,
                                                                             poly_degree,
        ...
        // HPS scale and round
        context.gpu_rns_tool_vec()[chain_index].hps_decrypt_scale_and_round(destination.data(),
                                                                            inner_prod.get());
    } else {
...
void CAHELGPUSecretKey::bgv_decrypt(const CAHELGPUContext &context, const CAHELGPUCiphertext &encrypted,
                                    CAHELGPUPlaintext &destination) {
    if (!encrypted.is_ntt_form()) {
        throw invalid_argument("encrypted must be in NTT form");
    }
    ...
    if (encrypted.correction_factor() != 1) {
        ...
        multiply_scalar_rns_poly<<<gridDimGlb, blockDimGlb>>>(destination.data(), fix,
                                                              context.gpu_plain_tables().modulus(),
                                                              destination.data(), poly_degree, 1);
    }
}

void CAHELGPUSecretKey::decrypt(const CAHELGPUContext &context, const CAHELGPUCiphertext &cipher, CAHELGPUPlaintext &plain) {
    auto cahel_context = context.cpu_context_;
    auto &context_data = (CAHELContext::ContextData &) (cahel_context->get_context_data(
            0)); // Use key_parm_id for obtaining scheme
    auto &parms = (EncryptionParameters &) (context_data.parms());
...
#ifdef CAHEL_DEBUG
    if (cipher.is_ntt_form_ && scheme == cahel::scheme_type::bfv)
    {
        throw invalid_argument("encrypted cannot be in NTT form");
    }
    if (!cipher.is_ntt_form_ && scheme == cahel::scheme_type::ckks)
    {
        throw invalid_argument("encrypted should be in NTT form");
    }
#endif
...
[[nodiscard]] int
CAHELGPUSecretKey::invariant_noise_budget(const CAHELGPUContext &context, const CAHELGPUCiphertext &cipher) {
    ...
    if (cipher.is_ntt_form_)
    {
        throw invalid_argument("cipher cannot be in NTT form");
    }
#endif
    ...
    auto base_q = context.cpu_context_->get_context_data_rns_tool(chain_index)->base_Ql();
    if (base_q == nullptr) {
        throw logic_error("base_Ql is null");
    }
