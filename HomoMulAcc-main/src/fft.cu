 ...
 * @param[inout] gpu_rns_vec_ The DRNSInfo stored in CAHELGPUContext.
...
void special_fft_forward(DCKKSEncoderInfo *gpu_ckks_msg_vec, uint32_t msg_vec_size)
{
    ...
    for (uint32_t twr = 0; twr < msg_vec_size; twr++)
    {
        ...
        if (gp->sparse_slots() <= SWITCH_POINT)
        {
            ...
            inplace_special_ffft_base_kernel<<<blocksPerGrid, threadsPerBlock, gp->sparse_slots() * sizeof(cuDoubleComplex), gp->SID()>>>(gp->in(), gp->twiddle(), gp->mul_group(), gp->sparse_slots(), logn, numOfGroups, iter, gp->m(), logRatio);
        }
        else
        {
            uint32_t iter = 0;
            ...
            {
                inplace_special_ffft_iter_kernel<<<blocksPerGrid, threadsPerBlock, 0, gp->SID()>>>(gp->in(), gp->twiddle(), gp->mul_group(), gp->sparse_slots(), logn, numOfGroups, iter, gp->m(), logRatio);

                iter++;
            }

            inplace_special_ffft_base_kernel<<<blocksPerGrid, threadsPerBlock, SWITCH_POINT * sizeof(cuDoubleComplex), gp->SID()>>>(gp->in(), gp->twiddle(), gp->mul_group(), gp->sparse_slots(), logn, numOfGroups, iter, gp->m(), logRatio);
        }
    }
 ...
 * @param[inout] gpu_rns_vec_ The vector<DRNSInfo> stored in CAHELGPUContext.
 * @param[in] coeff_mod_size The number of coeff modulus
 */
void special_fft_backward(DCKKSEncoderInfo *gpu_ckks_msg_vec, uint32_t msg_vec_size, double scalar)
{
    ...
    for (uint32_t twr = 0; twr < msg_vec_size; twr++)
    {
        ...
        if (gp->sparse_slots() <= SWITCH_POINT)
        {
            ...
            threadsPerBlock = gp->sparse_slots() >> 1;
            blocksPerGrid = 1;
            inplace_special_ifft_base_kernel<<<blocksPerGrid, threadsPerBlock, gp->sparse_slots() * sizeof(cuDoubleComplex), gp->SID()>>>(gp->in(), gp->twiddle(), gp->mul_group(), gp->sparse_slots(), logn, numOfGroups, iter, gp->m(), logRatio, scalar);
        }
        else
        {
            // printf("******************************************\n");
            ...
            inplace_special_ifft_base_kernel<<<blocksPerGrid, threadsPerBlock, SWITCH_POINT * sizeof(cuDoubleComplex), gp->SID()>>>(gp->in(), gp->twiddle(), gp->mul_group(), gp->sparse_slots(), logn, numOfGroups, iter, gp->m(), logRatio, scalar);
            numOfGroups >>= 1;
            for (; numOfGroups >= 1; numOfGroups >>= 1)
            {
                iter--;

                inplace_special_ifft_iter_kernel<<<blocksPerGrid, threadsPerBlock, 0, gp->SID()>>>(gp->in(), gp->twiddle(), gp->mul_group(), gp->sparse_slots(), logn, numOfGroups, iter, gp->m(), logRatio, scalar);
            }
        }
    }
    for (uint32_t twr = 0; twr < msg_vec_size; twr++)
