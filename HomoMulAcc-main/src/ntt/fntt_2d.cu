...
#pragma unroll
        ...
        //        twr_idx * n + n_init,
        //        twr_idx * n + n_init + t / 4,
        //        twr_idx * n + n_init + t / 4 * 2,
        //        twr_idx * n + n_init + t / 4 * 3,
        //        twr_idx * n + n_init + t / 4 * 4,
        //        twr_idx * n + n_init + t / 4 * 5,
        //        twr_idx * n + n_init + t / 4 * 6,
        //        twr_idx * n + n_init + t / 4 * 7);
        size_t tw_idx = 1;
        // printf("threadid = %4d: %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n", tid,
        //        tw_idx, tw_idx, tw_idx, tw_idx,
        //        2 * tw_idx, 2 * tw_idx, 2 * tw_idx + 1, 2 * tw_idx + 1,
        //        4 * tw_idx, 4 * tw_idx + 1, 4 * tw_idx + 2, 4 * tw_idx + 3);
...
#pragma unroll
            ...
            if (j == group / 4)
                remain_iters = 2;
            // printf("threadid = %4d: %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n", tid,
            //        tw_idx, tw_idx, tw_idx, tw_idx,
            //        2 * tw_idx, 2 * tw_idx, 2 * tw_idx + 1, 2 * tw_idx + 1,
            //        4 * tw_idx, 4 * tw_idx + 1, 4 * tw_idx + 2, 4 * tw_idx + 3,
            //        tw_idx2, tw_idx2, tw_idx2, tw_idx2,
            //        2 * tw_idx2, 2 * tw_idx2, 2 * tw_idx2 + 1, 2 * tw_idx2 + 1,
            //        4 * tw_idx2, 4 * tw_idx2 + 1, 4 * tw_idx2 + 2, 4 * tw_idx2 + 3);
            __syncthreads();
        }
...
inplace_fnwt_radix8_phase1_include_temp_mod(uint64_t *inout,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        // base address
        uint64_t *data_ptr = inout + twr_idx * n;
        const DMulUIntMod *psi = twiddles + twr_idx2 * n;
        const DModulus *modulus_table = modulus;
...
inplace_fnwt_radix8_phase2_include_temp_mod(uint64_t *inout,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        // base address
        uint64_t *data_ptr = inout + twr_idx * n;
        const DMulUIntMod *psi = twiddles + n * twr_idx2;
        const DModulus *modulus_table = modulus;
        uint64_t modulus = modulus_table[twr_idx2].value();
        size_t n_init = 2 * m_idx * t + t_idx;
#pragma unroll
...
__global__ static void
inplace_fnwt_radix8_phase1_single_mod(uint64_t *inout,
                                      const DMulUIntMod *twiddles,
                                      const DModulus *modulus,
                                      size_t coeff_mod_size,
                                      size_t start_mod_idx,
...
void nwt_2d_radix8_forward_inplace_include_temp_mod(uint64_t *inout,
    ...
    const size_t per_thread_sample_size = 8;
    // per_block_pad can be 1, 2, 4, etc., per_block_pad * phase1_sample_size / 8 <= blockDim.x
    // per_block_pad = 4 or 8 seems to be most optimized
    // for n=4096, max pad = 8
    const size_t per_block_pad = 8;
    const size_t per_block_memory = blockDimNTT.x * per_thread_sample_size * sizeof(uint64_t);
    //
    ...
    (phase1_sample_size + per_block_pad + 1) * per_block_pad * sizeof(uint64_t)>>>(
            ...
            per_block_pad);
    // max 512 threads per block
    inplace_fnwt_radix8_phase2_include_temp_mod<<<gridDimNTT, blockDimNTT, per_block_memory>>>(
            inout,
            ntt_tables.twiddle(),
            ntt_tables.modulus(),
            coeff_modulus_size,
...
void nwt_2d_radix8_forward_inplace_include_special_mod(uint64_t *inout,
    ...
    (phase1_sample_size + per_block_pad + 1) * per_block_pad * sizeof(uint64_t)>>>(
            ...
            per_block_pad);
    // max 512 threads per block
    inplace_fnwt_radix8_phase2_include_special_mod<<<gridDimNTT, blockDimNTT, per_block_memory>>>(
            inout,
            ntt_tables.twiddle(),
            ntt_tables.modulus(),
            coeff_modulus_size,
...
void nwt_2d_radix8_forward_inplace_single_mod(uint64_t *inout,
    ...
    const size_t per_block_memory = blockDimNTT.x * per_thread_sample_size * sizeof(uint64_t);
    //
    inplace_fnwt_radix8_phase1_single_mod<<<
    gridDimNTT,
    (phase1_sample_size / 8) * per_block_pad,
    (phase1_sample_size + per_block_pad + 1) * per_block_pad * sizeof(uint64_t)>>>(
            inout,
            ...
            modulus_index);
    // max 512 threads per block
    inplace_fnwt_radix8_phase2_single_mod<<<gridDimNTT, blockDimNTT, per_block_memory>>>(
            inout,
            ntt_tables.twiddle(),
            ntt_tables.modulus(),
            coeff_modulus_size,
