...
namespace CAHELtest
{
    namespace util
    {
        ...
        TEST(Common, SafeArithmetic)
        {
            ...
            ASSERT_THROW(sub_safe(unsigned(0), pos_u), logic_error);
            ASSERT_THROW(sub_safe(unsigned(4), pos_u), logic_error);
            ASSERT_THROW(add_safe(pos_uc_max, (unsigned char)1), logic_error);
            ASSERT_TRUE(pos_uc_max == add_safe(pos_uc_max, (unsigned char)0));
            ASSERT_THROW(mul_safe(pos_ull_max, pos_ull_max), logic_error);
            ASSERT_EQ(0ULL, mul_safe(0ULL, pos_ull_max));
            ASSERT_TRUE((long long)1 == mul_safe(neg_ull, neg_ull));
            ASSERT_THROW(mul_safe(pos_uc_max, pos_uc_max), logic_error);
        ...
        {
            ASSERT_EQ(static_cast<uint64_t>(0), reverse_bits(static_cast<uint64_t>(0ULL)));
            ASSERT_EQ(static_cast<uint64_t>(1ULL << 63), reverse_bits(static_cast<uint64_t>(1ULL)));
            ASSERT_EQ(static_cast<uint64_t>(1ULL << 32), reverse_bits(static_cast<uint64_t>(1ULL << 31)));
            ASSERT_EQ(static_cast<uint64_t>(0xFFFFULL << 32), reverse_bits(static_cast<uint64_t>(0xFFFFULL << 16)));
            ASSERT_EQ(static_cast<uint64_t>(0x0000FFFFFFFF0000ULL), reverse_bits(static_cast<uint64_t>(0x0000FFFFFFFF0000ULL)));
            ASSERT_EQ(static_cast<uint64_t>(0x0000FFFF0000FFFFULL), reverse_bits(static_cast<uint64_t>(0xFFFF0000FFFF0000ULL)));
