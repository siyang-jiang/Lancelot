...
from utils.openfhe import set_parameters, computing_distance_cipher, decrypt_sort_mask, mul_mask_weight
...
def GPU_mul_mask_weight(context,pk,encoder,scale,rlk,vectors_final,mask,sk):
    ...
    print(f"Mask Multiplication Time is: {end - start:.6f}")
...
def krum(w_locals, c, args, cryptocontext, keypair):
    ...
    if args.openfhe:
        ...
        vectors_final = vectors_final_new
        end = time.time()
        round_time = end - start


    print(f"We select {sorted_idx[0]} as our aggerated client.")
    return copy.deepcopy(w_locals[chosen_idx]), chosen_idx, round_time
